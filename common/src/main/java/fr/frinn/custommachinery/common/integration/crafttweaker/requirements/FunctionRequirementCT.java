...
public interface FunctionRequirementCT<T> extends RecipeCTBuilder<T> {
    ...
    default T requireFunctionToStart(Function<Context, CraftingResult> function) {
        return addRequirement(new FunctionRequirement(FunctionRequirement.Phase.CHECK, new CTFunction(function), CraftTweakerAPI.LOGGER::throwing));
    }

    @Method
    default T requireFunctionOnStart(Function<Context, CraftingResult> function) {
        return addRequirement(new FunctionRequirement(FunctionRequirement.Phase.START, new CTFunction(function), CraftTweakerAPI.LOGGER::throwing));
    }

    @Method
    default T requireFunctionEachTick(Function<Context, CraftingResult> function) {
        return addRequirement(new FunctionRequirement(FunctionRequirement.Phase.TICK, new CTFunction(function), CraftTweakerAPI.LOGGER::throwing));
    }

    @Method
    default T requireFunctionOnEnd(Function<Context, CraftingResult> function) {
        return addRequirement(new FunctionRequirement(FunctionRequirement.Phase.END, new CTFunction(function), CraftTweakerAPI.LOGGER::throwing));
    }
}
