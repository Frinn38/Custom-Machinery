...
public class ExperienceRequirement extends AbstractChanceableRequirement<ExperienceMachineComponent> implements IJEIIngredientRequirement<ExperienceStorage> {
  public static final NamedCodec<ExperienceRequirement> CODEC = NamedCodec.record(experienceRequirementInstance ->
    experienceRequirementInstance.group(
      ...
      experienceRequirementInstance, (mode, amount, chance) -> {
        ExperienceRequirement requirement = new ExperienceRequirement(mode, amount);
  ...
  public ExperienceRequirement(RequirementIOMode mode, float amount) {
    super(mode);
    this.amount = amount;
  }
  ...
  public CraftingResult processStart(ExperienceMachineComponent component, ICraftingContext context) {
    ...
    if (getMode() == RequirementIOMode.INPUT) {
      ...
      return CraftingResult.error(Component.translatable("custommachinery.requirements.xp.error.input", amount, canExtract));
    }
  ...
  public CraftingResult processEnd(ExperienceMachineComponent component, ICraftingContext context) {
    ...
    if (getMode() == RequirementIOMode.OUTPUT) {
      ...
      return CraftingResult.error(Component.translatable("custommachinery.requirements.xp.error.output", amount));
    }
    return CraftingResult.pass();
  }
  ...
  public List<IJEIIngredientWrapper<ExperienceStorage>> getJEIIngredientWrappers(IMachineRecipe recipe) {
    return Collections.singletonList(new ExperienceIngredientWrapper(this.getMode(), this.amount, getChance(), false, recipe.getRecipeTime()));
  }
}
