...
public interface DropRequirementCT<T> extends RecipeCTBuilder<T> {
    ...
    default T checkDrop(IItemStack item, int radius) {
        return checkDrops(new IItemStack[]{item}, item.amount(), radius, true);
    }
    ...
    default T consumeDropOnStart(IItemStack item, int radius) {
        return consumeDropsOnStart(new IItemStack[]{item}, item.amount(), radius, true);
    }
    ...
    default T consumeDropOnEnd(IItemStack item, int radius) {
        return consumeDropsOnEnd(new IItemStack[]{item}, item.amount(), radius, true);
    }
    ...
    default T dropItemOnStart(IItemStack stack) {
        return addRequirement(new DropRequirement(RequirementIOMode.INPUT, DropRequirement.Action.PRODUCE, Collections.emptyList(), true, stack.getDefinition(), CTUtils.nbtFromStack(stack), stack.amount(), 1));
    }

    @Method
    default T dropItemOnEnd(IItemStack stack) {
        return addRequirement(new DropRequirement(RequirementIOMode.OUTPUT, DropRequirement.Action.PRODUCE, Collections.emptyList(), true, stack.getDefinition(), CTUtils.nbtFromStack(stack), stack.amount(), 1));
    }
}
