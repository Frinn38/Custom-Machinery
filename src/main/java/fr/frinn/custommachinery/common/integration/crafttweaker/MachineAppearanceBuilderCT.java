...
public class MachineAppearanceBuilderCT {
    ...
    public MachineAppearanceBuilderCT toolType(String[] tools) {
        List<TagKey<Block>> list = Arrays.stream(tools).map(key -> TagKey.create(Registries.BLOCK, ResourceLocation.parse(key))).toList();
    ...
    public MachineAppearanceBuilderCT miningLevel(String key) {
        TagKey<Block> level = TagKey.create(Registries.BLOCK, ResourceLocation.parse(key));
