...
public interface DurabilityRequirementJS extends RecipeJSBuilder {
    ...
    default RecipeJSBuilder damageItemTag(String tag, int amount, Map<?,?> nbt, String slot) {
        try {
            return this.addRequirement(new DurabilityRequirement(RequirementIOMode.INPUT, ItemTagIngredient.create(tag), amount, null, true, slot));
    ...
    default RecipeJSBuilder damageItemTagNoBreak(String tag, int amount, Map<?,?> nbt, String slot) {
        try {
            return this.addRequirement(new DurabilityRequirement(RequirementIOMode.INPUT, ItemTagIngredient.create(tag), amount, null, false, slot));
        } catch (IllegalArgumentException e) {
            return error(e.getMessage());
        }
    }
    ...
    default RecipeJSBuilder repairItemTag(String tag, int amount, Map<?,?> nbt, String slot) {
        try {
            return this.addRequirement(new DurabilityRequirement(RequirementIOMode.OUTPUT, ItemTagIngredient.create(tag), amount, null, false, slot));
        } catch (IllegalArgumentException e) {
            return error(e.getMessage());
        }
    }
