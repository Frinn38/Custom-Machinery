...
public class CustomMachineRecipeSerializer extends ForgeRegistryEntry<IRecipeSerializer<?>> implements IRecipeSerializer<CustomMachineRecipe> {
    ...
    public CustomMachineRecipe read(ResourceLocation recipeId, JsonObject json) {
        CustomMachineryAPI.getLogger().info("Parsing recipe json: %s", recipeId);
        ...
        if(result.result().isPresent()) {
            CustomMachineryAPI.getLogger().info("Successfully read recipe json: %s", recipeId);
            return result.result().get().getFirst().build(recipeId);
        } else if(result.error().isPresent()) {
            CustomMachineryAPI.getLogger().error("Error while parsing recipe json: %s, skipping...%n%s", recipeId, result.error().get().message());
    ...
    public CustomMachineRecipe read(ResourceLocation recipeId, PacketBuffer buffer) {
        CustomMachineryAPI.getLogger().info("Receiving recipe: %s from server.", recipeId);
        ...
        if(result.result().isPresent()) {
            CustomMachineryAPI.getLogger().info("Sucessfully received recipe: %s from server.", recipeId);
    ...
    public void write(PacketBuffer buffer, CustomMachineRecipe recipe) {
        CustomMachineryAPI.getLogger().info("Sending recipe: %s to clients", recipe.getId());
        ...
        if(result.result().isPresent()) {
            CustomMachineryAPI.getLogger().info("Sucessfully send recipe: %s to clients.", recipe.getId());
            buffer.writeCompoundTag((CompoundNBT) result.result().get());
            return;
        } else if(result.error().isPresent()) {
            CustomMachineryAPI.getLogger().error("Error while sending recipe: %s to clients.%n%s", recipe.getId(), result.error().get().message());
