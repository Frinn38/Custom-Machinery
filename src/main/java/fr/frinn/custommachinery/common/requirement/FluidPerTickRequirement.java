...
public record FluidPerTickRequirement(RequirementIOMode mode, SizedFluidIngredient ingredient, String tank) implements IRequirement<FluidComponentHandler>, IJEIIngredientRequirement<FluidStack> {
    ...
    public List<IJEIIngredientWrapper<FluidStack>> getJEIIngredientWrappers(IMachineRecipe recipe, RecipeRequirement<?, ?> requirement) {
        return Collections.singletonList(new FluidIngredientWrapper(this.getMode(), this.ingredient, requirement.chance(), true, this.tank));
    }
}
