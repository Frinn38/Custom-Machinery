...
public record ExperienceRequirement(RequirementIOMode mode, int amount, Form form) implements IRequirement<ExperienceMachineComponent>, IJEIIngredientRequirement<Experience> {
    ...
    public static final NamedCodec<ExperienceRequirement> CODEC = NamedCodec.record(experienceRequirementInstance ->
            experienceRequirementInstance.group(
                    RequirementIOMode.CODEC.fieldOf("mode").forGetter(ExperienceRequirement::getMode),
                    NamedCodec.INT.fieldOf("amount").forGetter(ExperienceRequirement::amount),
                    NamedCodec.enumCodec(Form.class).optionalFieldOf("form", Form.POINT).forGetter(ExperienceRequirement::form)
            ).apply(experienceRequirementInstance, ExperienceRequirement::new), "Experience requirement"
    );

    @Override
    public RequirementType<ExperienceRequirement> getType() {
        return CMRegistration.EXPERIENCE_REQUIREMENT.get();
    }
