...
public record MachineUpgrade(Item item, List<Identifier> machines, List<RecipeModifier> recipeModifiers,
    ...
    public static final NamedCodec<MachineUpgrade> CODEC = NamedCodec.record(machineUpgradeInstance ->
            machineUpgradeInstance.group(
                    RegistryCodecs.ITEM.fieldOf("item").forGetter(upgrade -> upgrade.item),
                    DefaultCodecs.IDENTIFIER.listOf().fieldOf("machines").forGetter(upgrade -> upgrade.machines),
