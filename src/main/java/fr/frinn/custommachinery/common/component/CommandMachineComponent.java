...
public class CommandMachineComponent extends AbstractMachineComponent {
    ...
    public void sendCommand(String command, int permissionLevel, boolean log) {
        ...
        CommandSourceStack source = new CommandSourceStack(log ? COMMAND_SOURCE_LOG : COMMAND_SOURCE_NO_LOG, Vec3.atCenterOf(getManager().getTile().getBlockPos()), getMachineRotation(), (ServerLevel)getManager().getLevel(), PermissionSet.ALL_PERMISSIONS, "Custom Machinery", getManager().getTile().getMachine().getName(), getManager().getServer(), null);
