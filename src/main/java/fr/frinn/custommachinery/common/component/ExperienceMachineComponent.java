...
public class ExperienceMachineComponent extends AbstractMachineComponent implements ITickableComponent, ISerializableComponent, ISyncableStuff {
    ...
    public ExperienceMachineComponent(IMachineComponentManager manager, int capacity, boolean retrieveFromSlots, List<String> slotIds) {
        ...
        this.capacity = this.upgradeableI(capacity, "capacity", 1, Integer.MAX_VALUE, value -> {
            this.capacityLevels = ExperienceUtils.getLevelFromXp(value);
            this.xp = Math.min(this.xp, value);
            this.xpLevels = ExperienceUtils.getLevelFromXp(this.xp);
        });
    ...
    public void setXp(int xp) {
        this.xp = Mth.clamp(xp, 0, this.getCapacity());
    ...
    public void getStuffToSync(Consumer<ISyncable<?, ?>> container) {
        ...
        container.accept(IntegerSyncable.create(this::getCapacityLevels, capacityLevels -> this.capacityLevels = capacityLevels));
    }
    ...
    public void addLevelToPlayer(int levelDiff, Player player) {
        ...
        int awardXP = levelDiff > 0 ? Math.min(this.xp, requestedXP) : -Math.min(Math.abs(requestedXP), this.getCapacity() - this.xp);
    ...
    public void addAllLevelToPlayer(boolean give, Player player) {
        ...
        if (give) {
            ...
            awardXP = -Math.min(ExperienceUtils.getPlayerTotalXp(player), this.getCapacity() - this.xp);
        }
        awardXP(awardXP, player);
    }
