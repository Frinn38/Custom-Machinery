...
public abstract class AbstractGuiElement implements IGuiElement {
    ...
    public record Properties(int x, int y, int width, int height, int priority, @Nullable TextureInfo texture, @Nullable TextureInfo textureHovered, List<Component> tooltips, String id){}
    ...
    public static NamedMapCodec<Properties> makePropertiesCodec(@Nullable TextureInfo defaultTexture) {
        return makePropertiesCodec(defaultTexture, null, Collections.emptyList());
    }
    ...
    public static NamedMapCodec<Properties> makePropertiesCodec(@Nullable TextureInfo defaultTexture, @Nullable TextureInfo defaultTextureHovered) {
        return makePropertiesCodec(defaultTexture, defaultTextureHovered, Collections.emptyList());
    }
    ...
    public static NamedMapCodec<Properties> makePropertiesCodec(@Nullable TextureInfo defaultTexture, @Nullable TextureInfo defaultTextureHovered, List<Component> defaultTooltips) {
        return NamedCodec.record(propertiesInstance ->
             propertiesInstance.group(
                     ...
                     TextureInfo.CODEC.optionalFieldOf("texture").forGetter(properties -> defaultTexture != null && defaultTexture.equals(properties.texture()) ? Optional.empty() : Optional.ofNullable(properties.texture())),
                     TextureInfo.CODEC.optionalFieldOf("texture_hovered").forGetter(properties -> defaultTextureHovered != null && defaultTextureHovered.equals(properties.textureHovered()) ? Optional.empty() : Optional.ofNullable(properties.textureHovered())),
