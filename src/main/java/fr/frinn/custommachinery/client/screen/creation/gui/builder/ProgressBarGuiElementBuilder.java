...
public class ProgressBarGuiElementBuilder implements IGuiElementBuilder<ProgressBarGuiElement> {
    ...
    public ProgressBarGuiElement make(Properties properties, @Nullable ProgressBarGuiElement from) {
        if(from != null)
            return new ProgressBarGuiElement(properties, from.getEmptyTexture(), from.getFilledTexture(), from.getDirection(), from.getStart(), from.getEnd());
        else
            return new ProgressBarGuiElement(properties, ProgressBarGuiElement.BASE_EMPTY_TEXTURE, ProgressBarGuiElement.BASE_FILLED_TEXTURE, Orientation.RIGHT, 0.0F, 0.0F);
    }
