...
public class BarGuiElementBuilder implements IGuiElementBuilder<BarGuiElement> {
    ...
    public GuiElementType<BarGuiElement> type() {
        return CMRegistration.BAR_GUI_ELEMENT.get();
    }
    ...
    public static class BarGuiElementBuilderPopup extends GuiElementBuilderPopup<BarGuiElement> {
        ...
        public void addWidgets(RowHelper row) {
            ...
            this.highlight = row.addChild(Checkbox.builder(Component.translatable("custommachinery.gui.creation.gui.highlight"), this.font).selected(this.baseElement == null || this.baseElement.isHighlight()).build());
            row.addChild(new StringWidget(Component.translatable("custommachinery.gui.creation.gui.bar.orientation"), this.font));
            this.orientation = row.addChild(CycleButton.<Orientation>builder(orientation -> Component.literal(orientation.name()), this.baseElement != null ? this.baseElement.getOrientation() : Orientation.TOP).withValues(Orientation.values()).displayOnlyValue().create(0, 0, 100, 20, Component.translatable("custommachinery.gui.creation.gui.bar.orientation")));
        }
    }
}
