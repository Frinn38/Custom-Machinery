...
public class RedstoneComponentBuilder implements IMachineComponentBuilder<RedstoneMachineComponent, Template> {
    ...
    public void render(GuiGraphicsExtractor graphics, int x, int y, int width, int height, Template template) {
        graphics.item(Items.REDSTONE.getDefaultInstance(), x, y + height / 2 - 8);
    ...
    public static class RedstoneComponentBuilderPopup extends ComponentBuilderPopup<Template> {
        ...
        protected void init() {
            ...
            //ComparatorInputType
            this.comparatorInputType = this.propertyList.add(Component.translatable("custommachinery.gui.creation.components.redstone.comparatorInputType"), CycleButton.<MachineComponentType<?>>builder(type -> Component.literal(type.getId().toString()), CMRegistration.ENERGY_MACHINE_COMPONENT.get()).displayOnlyValue().withValues(ImmutableList.copyOf(CMRegistration.MACHINE_COMPONENT_TYPE_REGISTRY)).create(0, 0, 150, 20, Component.translatable("custommachinery.gui.creation.components.redstone.comparatorInputType")));
